...
import {
  ...
  ToastiveVariant,
  type ToastiveProps
} from '../../src/toastive.types'
import type { ToastiveInstance } from '../../src'
import { clearToasts, resetDefaults, setDefaults, showToastive } from '../../src/showToastive'
...
/**
 * Scope a click handler to a temporary `setDefaults()` block.
 * Applies the given defaults, runs `fn`, then restores whatever defaults
 * were in place — even if `fn` throws. Keeps the demos here from leaking
 * state into other stories.
 */
const withDefaults = <T>(props: ToastiveDefaults, fn: () => T): T => {
  setDefaults(props)
  try {
    return fn()
  } finally {
...
const playgroundButton = (
  ...
  btn.classList.add('playground')
  if (variant === 'muted') btn.classList.add('playground--muted')
  btn.type = 'button'
  btn.addEventListener('click', onClick)
  return btn
}

const playgroundRow = (...children: HTMLElement[]): HTMLDivElement => {
  const row = document.createElement('div')
  row.style.cssText =
    'display: flex; flex-wrap: wrap; gap: 0.5rem; align-items: center; justify-content: center; padding: 0.5rem;'
  children.forEach((c) => row.append(c))
  return row
}

const withMessage = (variant: ToastiveVariant, message?: string): ToastiveProps => ({
  title: variant.charAt(0).toUpperCase() + variant.slice(1),
  message: message ?? `This is a ${variant} toast`,
  variant,
  autoClose: true,
  duration: 5000
})
...
const meta: Meta<ToastiveProps> = {
  argTypes: {
    ...
    autoClose: {
      ...
      description: 'Auto close the toast after `duration`',
      name: 'Auto Close',
      table: { category: 'Behavior' }
    },
    closeButton: {
    ...
    customIcon: {
      control: { type: 'text' },
      description: 'HTML string for the icon slot default content',
      name: 'Custom Icon',
      table: { category: 'Appearance' }
    },
    draggable: {
      control: { type: 'boolean' },
      description: 'Allow the toast to be dismissed by dragging',
      name: 'Draggable',
      table: { category: 'Behavior' }
    },
    duration: {
      control: { type: 'number' },
      description: 'Milliseconds before auto-close (only if autoClose)',
      name: 'Duration',
      table: { category: 'Behavior' }
    },
    message: {
      control: { type: 'text' },
      description: 'Message body of the toast',
      name: 'Message',
      table: { category: 'Content' }
    },
    pauseOnHover: {
      control: { type: 'boolean' },
      description: 'Pause the timer when the toast is hovered or focused',
      name: 'Pause On Hover',
      table: { category: 'Behavior' }
    },
    position: {
    ...
    theme: {
      ...
      name: 'Theme',
      options: Object.values(ToastiveTheme),
      table: { category: 'Appearance' }
    },
    title: {
      control: { type: 'text' },
...
export const playground: StoryObj<ToastiveProps> = {
...
export const quickStart: StoryObj = {
  render: () =>
    playgroundRow(
      playgroundButton('Show Toastive 🚀', () =>
        showToastive({
          title: 'Saved',
...
// Combined row of all 6 variants — lives in one iframe so the buttons
// share a single flex row instead of stacking as separate stories.
export const variants: StoryObj = {
  render: () =>
    playgroundRow(
      playgroundButton('Default', () => showToastive(withMessage(ToastiveVariant.Default))),
      playgroundButton('Success', () => showToastive(withMessage(ToastiveVariant.Success))),
      playgroundButton('Info', () => showToastive(withMessage(ToastiveVariant.Info))),
      playgroundButton('Warning', () => showToastive(withMessage(ToastiveVariant.Warning))),
      playgroundButton('Error', () => showToastive(withMessage(ToastiveVariant.Error))),
      playgroundButton('Loading', () =>
        showToastive({
...
export const defaultVariant: StoryObj = {
  render: () =>
    playgroundRow(
      playgroundButton('Show default toast', () =>
        showToastive(withMessage(ToastiveVariant.Default))
      )
    ),
  tags: ['auto-docs', '!dev']
}
...
export const successVariant: StoryObj = {
  render: () =>
    playgroundRow(
      playgroundButton('Show success toast', () =>
        showToastive(withMessage(ToastiveVariant.Success))
      )
    ),
  tags: ['auto-docs', '!dev']
}
...
export const infoVariant: StoryObj = {
  render: () =>
    playgroundRow(
      playgroundButton('Show info toast', () => showToastive(withMessage(ToastiveVariant.Info)))
    ),
  tags: ['auto-docs', '!dev']
}
...
export const warningVariant: StoryObj = {
  render: () =>
    playgroundRow(
      playgroundButton('Show warning toast', () =>
        showToastive(withMessage(ToastiveVariant.Warning))
      )
    ),
  tags: ['auto-docs', '!dev']
}
...
export const errorVariant: StoryObj = {
  render: () =>
    playgroundRow(
      playgroundButton('Show error toast', () => showToastive(withMessage(ToastiveVariant.Error)))
    ),
  tags: ['auto-docs', '!dev']
}

export const loadingVariant: StoryObj = {
  render: () =>
    playgroundRow(
      playgroundButton('Show loading toast', () =>
        showToastive({
          ...withMessage(ToastiveVariant.Loading, 'Loading...'),
          autoClose: false
        })
...
export const themes: StoryObj = {
  render: () => {
    const showLight = playgroundButton('Light theme', () =>
      showToastive({
    ...
    const showDark = playgroundButton('Dark theme', () =>
      showToastive({
        variant: ToastiveVariant.Info,
...
export const pauseOnHover: StoryObj = {
  render: () => {
    const withPause = playgroundButton('With pause (15s)', () =>
      showToastive({
    ...
    const withoutPause = playgroundButton('Without pause (15s)', () =>
      showToastive({
...
export const maxVisible: StoryObj = {
  render: () => {
    ...
    const spam = playgroundButton('Spam 6 toasts (cap: 3 per position)', () =>
      withDefaults({ maxVisible: 3 }, () => {
        for (let i = 0; i < 6; i++) {
          showToastive({
            message: `Toast #${i + 1}`,
            variant: ToastiveVariant.Info
...
// ---------------------------------------------------------------------------
// Update in place (loading -> success)
// ---------------------------------------------------------------------------

export const updateInPlace: StoryObj = {
  render: () => {
...
export const globalDefaults: StoryObj = {
  render: () => {
    ...
    const success = playgroundButton('Success (uses defaults)', () =>
      withDefaults(appDefaults, () =>
        showToastive({
          variant: ToastiveVariant.Success,
    ...
    const error = playgroundButton('Error (uses defaults)', () =>
      withDefaults(appDefaults, () =>
        showToastive({
...
export const clearingToasts: StoryObj = {
  render: () => {
    ...
    const clearTopRight = playgroundButton('Clear top-right only', () =>
      clearToasts(ToastivePosition.TopRight)
    )
...
export const customIcon: StoryObj = {
  render: () => {
    ...
    const showRocket = playgroundButton('Show with custom rocket icon', () =>
      showToastive({
        variant: ToastiveVariant.Info,
    ...
    const showStar = playgroundButton('Show with custom star icon', () =>
      showToastive({
        variant: ToastiveVariant.Success,
